...
namespace MyTrainer
{
    public partial class changeObjective_pe : Form
    {
        ...
        bool zerotime_condition, isAnyValChanged;

        public changeObjective_pe(trainerBase.TrainKind kindOfTrain)
        {
            InitializeComponent();
            frameSize = Size;
            isAnyValChanged = false;
        ...
        public void applyChanges(trainerBase.TrainKind trainKind)
        {
            ...
            if (trainKind.Equals(trainerBase.TrainKind.PhysicalEdu))
            {
                ...
                {
                    if (MessageBox.Show("Если ты сейчас отключишь функцию \"Цель\", то твои последние достижения будут сброшены. Это не повлияет на Статистику. Ты уверен?", "Тренировки", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
                    {
                        hasObjective.Checked = true;
                        MessageBox.Show("Достижения не были сброшены", "Тренировки", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Достижения по тренировкам сброшены", "Тренировки", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                if (hasObjective.Checked) // IF USER HAS DAILY/MONTHLY/ANY OBJECTIVE...
                {
                    ...
                    else // ELSE, IF TIME VALUE IS DIFERRENT TO 00:00:OO
                    {
                        ...
                        if (MessageBox.Show("Если ты изменишь свою цель, твой текущей прогресс в выбранной категории будет сброшен! Ты уверен?", "Тренировки", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                        {
                            ...
                            trainerBase.PE.TrainedToday = false;
                        }
                        else
                            MessageBox.Show("Прогресс не был сброшен", "Тренировки", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            else
            {
                ...
                {
                    if (MessageBox.Show("Если ты сейчас отключишь функцию \"Цель\", то твои последние достижения будут сброшены. Это не повлияет на Статистику. Ты уверен?", "Футбол", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.No)
                    {
                        hasObjective.Checked = true;
                        MessageBox.Show("Достижения не были сброшены", "Футбол", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    else
                    {
                        MessageBox.Show("Достижения по футболу сброшены", "Футбол", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                if (hasObjective.Checked) // IF USER HAS DAILY/MONTHLY/ANY OBJECTIVE...
                {
                    ...
                    else // ELSE, IF TIME VALUE IS DIFERRENT TO 00:00:OO
                    {
                        ...
                        if (MessageBox.Show("Если ты изменишь свою цель, твой текущей прогресс в выбранной категории будет сброшен! Ты уверен?", "Футбол", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
                        {
                            ...
                            trainerBase.Football.TrainedToday = false;
                        }
                        else
                            MessageBox.Show("Прогресс не был сброшен", "Футбол", MessageBoxButtons.OK, MessageBoxIcon.Information);
